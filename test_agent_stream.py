...
def test_extract_partial_hides_tool_calls():
    raw = '{"tool": "sql_select", "arguments": {"sql": "SELECT 1"}}'
...
async def test_agent_on_stream_skips_tool_json(monkeypatch):
    ...
    def _fake_stream(messages):
        yield '{"tool": "sql_select", "arguments": {"sql": "SELECT 1 as n"}}'
    ...
    assert not any("sql_select" in s for s in seen)
...
def test_tool_result_message_is_user_role():
    msg = agent._tool_result_message("sql_select", {"rows": [{"n": 1}]})
    assert msg["role"] == "user"
    assert msg["content"].startswith("TOOL RESULT (sql_select):")
